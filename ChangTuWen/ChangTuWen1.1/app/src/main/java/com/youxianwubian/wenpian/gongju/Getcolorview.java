...
public class Getcolorview extends View
{
	...
	private int mInitialColor=0xffffffff;//原始颜色

	public Getcolorview(Context context, int height, int width) {
		super(context);
	...
	float texth,textw;
	//设置预览文字大小
	public void settextzj(int size)
	{
		mCenterPaint.setTextSize(size);//设置文字大小
		
		Rect textBounds = new Rect(); 
		mCenterPaint.getTextBounds("预览", 0, "预览".length(), textBounds);//get text bounds, that can get the text width and height  
		Paint.FontMetricsInt fontMetrics = mCenterPaint.getFontMetricsInt();
		 texth= - fontMetrics.top/2;
	...
	protected void onDraw(Canvas canvas) {
		...
		{
			//画预览
			canvas.drawText("预览",-textw/2,texth,mCenterPaint);
		}else
		{
			//画中心圆
